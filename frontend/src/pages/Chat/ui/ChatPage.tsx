...
export const ChatPage = () => {
  ...
  const [memberStatus, setMemberStatus] = useState<'ACTIVE' | 'BAN' | null>(null)
  ...
  }, [selectedRoom, isConnected, isMember, memberStatus, clearMessages, joinRoom, leaveRoom, joinRoomMember])
  ...
  const handleSendMessage = (e: React.FormEvent) => {
    e.preventDefault()
    if (messageText.trim() && isConnected && selectedRoom && memberStatus === 'ACTIVE') {
      sendMessage({
              ...
              <div className={styles.chatHeader}>
                ...
                {isMember && (
                  <div className={styles.membersSection}>
                    <h4 className={styles.membersTitle}>Участники ({members.length})</h4>
                    <div className={styles.membersList}>
                      {members.map((member) => (
                        <div key={member.id} className={styles.memberItem}>
                          <span className={styles.memberUserId}>
                            {member.user_id === user?.id ? 'Вы' : member.user_id.slice(0, 8)}
                          </span>
                          <span
                            className={
