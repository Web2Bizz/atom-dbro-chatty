...
function ApiKeysPage() {
  ...
  const handleRevokeApiKey = async (id: string) => {
    if (
      !confirm(
        'Вы уверены, что хотите отозвать этот токен? После отзыва токен перестанет работать.',
      )
    ) {
      return
    }
      ...
      <div className='api-keys-container'>
        ...
        <div className='api-keys-content'>
          <div className='api-keys-info'>
            <p>
              API токены позволяют отправлять сообщения в чаты через REST API
              без использования веб-интерфейса. Создайте токен и используйте его
              в заголовке <code>X-API-Key</code> или{' '}
            ...
            <div className='api-keys-list'>
              {apiKeys.length === 0 ? (
                <div className='no-keys'>
                  ...
                  <p className='hint'>
                    Создайте первый токен, чтобы начать использовать API
                  </p>
                </div>
              ) : (
                apiKeys.map((key) => (
                  <div key={key.id} className='api-key-item'>
                    <div className='api-key-header'>
                      <div className='api-key-name'>
                        ...
                        <span
                          className={`status-badge ${key.isActive ? 'active' : 'revoked'}`}
                        >
