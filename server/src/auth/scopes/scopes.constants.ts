...
export enum Scope {
  ...
  /**
   * Разрешение на создание комнат
   * Позволяет создавать комнаты и иметь полный доступ к созданным комнатам
   * (просмотр, получение сообщений, отправка сообщений)
   * НЕ дает доступ ко всем комнатам системы, только к созданным этим API ключом
   */
  ALLOW_CREATE_ROOMS = 'allow-create-rooms',
}
...
export const SCOPE_METADATA: Record<Scope, { description: string; requiredScopes?: Scope[] }> = {
  [Scope.ALLOW_ALL]: {
    description:
      'Полный доступ ко всем функциям системы. Обладатель этого scope может выполнять любые действия.',
  },
  [Scope.ALLOW_ALL_CHATS]: {
    description:
      'Доступ ко всем операциям с чатами/комнатами системы. Позволяет создавать комнаты, просматривать все комнаты (включая приватные), получать историю сообщений и отправлять сообщения в любые комнаты.',
  },
  [Scope.ALLOW_ALL_USERS]: {
    description:
      'Доступ ко всем пользователям системы. Позволяет просматривать список всех пользователей, их профили и данные.',
  },
  [Scope.ALLOW_CREATE_ROOMS]: {
    description:
      'Разрешение на создание комнат. Позволяет создавать новые комнаты и иметь полный доступ к созданным комнатам (просмотр, получение истории сообщений, отправка сообщений). НЕ дает доступ ко всем комнатам системы, только к комнатам, созданным этим API ключом.',
  },
};
