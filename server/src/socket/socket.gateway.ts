...
export class SocketGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleConnection(client: Socket) {
    try {
      ...
      if (errorMessage.includes('jwt expired') || errorMessage.includes('token expired')) {
        this.logger.warn(`Client connection error: ${client.id} - ${error.message}`);
      } else {
        this.logger.error(`Client connection error: ${client.id}`, error.message);
      }
    }
  }
  ...
  async handleMessage(@ConnectedSocket() client: Socket, @MessageBody() data: any) {
    ...
    const canSend = await this.roomMembersService.canSendMessage(roomId, client.data.userId);
    if (!canSend) {
      this.logger.warn(`User ${client.data.userId} cannot send message to room ${roomId} - not an ACTIVE member`);
      client.emit('error', { message: 'You cannot send messages to this room. You may be banned or not a member.' });
      return;
    }

    this.logger.log(`Message from ${client.id} to room ${roomId}: ${JSON.stringify(data)}`);
  ...
  async handleJoinRoomMember(@ConnectedSocket() client: Socket, @MessageBody() data: { roomId: string }) {
    if (!client.data.userId) {
      throw new UnauthorizedException('User not authenticated');
    }
    ...
    try {
      ...
      this.logger.log(`Client ${client.id} (User: ${client.data.userId}) joined room as member: ${roomId}`);
      ...
      this.logger.error(`Error joining room ${roomId} as member: ${error.message}`);
