...
- **Все операции с чатами** требуют scope `allow-all-chats` или `allow-all` для API ключей
...
**Защищенные эндпоинты**: Все операции с чатами требуют scope `allow-all-chats` или `allow-all`. JWT пользователи имеют полный доступ автоматически.
